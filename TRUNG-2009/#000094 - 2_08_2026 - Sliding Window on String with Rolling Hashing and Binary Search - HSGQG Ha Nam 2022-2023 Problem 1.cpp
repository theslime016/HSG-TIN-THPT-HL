...
int main() {
    ...
    auto calc = [&](int left, int right) {
        return (hashS[right] - (hashS[left-1]*pow[right-left+1])%mod + mod)%mod;
    };

    auto calc2 = [&](int left, int right) {
        return (hash2[right] - (hash2[left-1]*pow2[right-left+1]%mod2) + mod2)%mod2;
    };
    ...
    if (carry > 0) otherSum += carry;
    if (otherSum > 0) cout << otherSum;
    cout << res;
